...
PATH = PATH[:-10] + '/Shared/Scenarios/Scenario2_cyborg--.yaml'
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
print('\n->device is', device)
print('->Scenario file is', PATH)
...
if __name__ == '__main__':
    ...
    action_space = [132, 133, 134, 138]  # restore enterprise and opserver
    action_space += [2, 3, 4, 8]  # analyse enterprise and opserver
    action_space += [15, 16, 17, 21]  # remove enterprise and opserer
    action_space += [10, 11, 12, 13]  # analyse user hosts
    action_space += [140, 141, 142, 143]  # restore user hosts
    action_space += [131]  # restore defender
    action_space += [1]  # analyse defender
    action_space += [14, 23, 24, 25, 26]  # remove defender and user hosts
